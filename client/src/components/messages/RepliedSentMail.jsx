...
export default function RepliedSentMail({ props }) {
  const { data } = props;
    ...
    <Box sx={{ padding: '4px 20px' }}>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: 370, border: '1px solid #f0f0f0' }}>
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              <TableCell sx={{ maxWidth: 70, height: 30, border: '1px solid #f0f0f0', fontWeight: 'bold', padding: '1px 2px 1px 8px', backgroundColor: '#f8f8f8', fontFamily: 'inherit', fontSize: '12px' }}>
                BOOKMARK
              </TableCell>
          ...
          <TableBody>
            {data.map((row, index) => (
              <TableRow hover role="checkbox" tabIndex={-1} key={index} onClick={() => handleRowClick(row)}>
                ...
                <TableCell sx={{ minWidth: 200, height: 30, padding: '1px 2px 1px 12px', border: '1px solid #f0f0f0', fontSize: '12px', fontFamily: 'inherit' }}>
