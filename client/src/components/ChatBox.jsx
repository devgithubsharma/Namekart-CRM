...
function ChatBox() {
        ...
        const fetchChat = async () => {
            try {
                ...
                const preparedChatHistory = emailData.map((email) => ({
                    ...
                    time: email.emailType === 'sent' ? new Date(email.sentTime).toISOString().replace('T', ' ').substring(0, 19) : 
                    new Date(email.receivedTime).toISOString().replace('T', ' ').substring(0, 19),
    ...
    const handleSendReply = async () => {
        ...
        setIsSending(true); 
        try {
            // const resp = await axios.post(`https://crmapi.namekart.com/api/sendReply`, {
            ...
            //     messageId: selectedChat[selectedChat.length-1].messageId,
            //     userId:userId
            // });
            const resp = await sendReply(selectedChat[0].sender_email,selectedChat[0].receiver_email,selectedChat[0].subject,replyText,selectedChat[0].campId,selectedChat[0].threadId,selectedChat[0].domainName,selectedChat[0].leads,selectedChat[selectedChat.length-1].messageId,userId)
            setReplyText('');
    ...
    return (
        <>
        <IconButton onClick={handleBackClick} sx={{ position: 'relative', top:0,left: 10, zIndex: 1400, color:"black",size:"2px" }}>
                <ArrowBackOutlinedIcon /> 
                <Typography variant="caption" sx={{ fontSize: '1.25rem',top:2 }}>Back</Typography>
        </IconButton>
        
        <Box sx={{ position: 'relative', display: 'flex', height: '100vh',padding: '16px', gap: '16px' }}>
                ...
                <List sx={{ overflowY: 'auto', maxHeight: 'calc(100vh - 100px)', padding: '16px' }}>
                    {inboxItems.map((item, index) => (
                        <Paper key={index} elevation={3} sx={{ mb: 2, bgcolor: '#f5f5f5',overflow: 'hidden', 
                            wordWrap: 'break-word', height: '150px'  }} onClick={() => handleInboxClick(item.threadId)} >
                        <ListItem button onClick={() => navigate(`/home/chats/${item.threadId}`)}>
                            <ListItemAvatar>
                            ...
                            <ListItemText
                                ...
                                secondary={
                                    <>
                                        ...
                                        <Typography variant="caption" color="textSecondary" sx={{ overflowY: 'auto', maxHeight: '80px', display: 'block' }}>{item.recentReply}</Typography>
                                    </>
                                }
            ...
            <Paper elevation={6} sx={{ width:"700px",flexGrow: 1, padding: '16px', display: 'flex', flexDirection: 'column' }}>
                    ...
                    {chatHistory.map((chat, index) => (
                        ...
                        <Paper
                            ...
                            sx={{
                                mb: 2,
                                p: 2,
                                backgroundColor: chat.receiver === 'You' ? '#eeeeee' : '#e1e16a',
                ...
                <Button variant="contained" color="primary" onClick={handleSendReply} sx={{ width: '150px',ml:67 }} disabled={isSending}>
                {isSending ? (
                    <>
