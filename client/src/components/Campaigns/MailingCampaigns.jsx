...
import { isValidSenderEmails } from "../../api";
...
function MailingCampaigns() {
  ...
  const [isEditSequenceDialogOpen, setIsEditSequenceDialogOpen] =
    useState(false);
  ...
  const stepIndex = sequences[sequenceIndex]?.steps.findIndex(
    ...
    return (
      <Dialog open={open} onClose={onCancel}>
        ...
        <DialogActions>
          ...
          <Button onClick={() => onDelete(sequence.id)}>Delete Sequence</Button>

          <Button
            onClick={() => onSave(sequence.id, name, isEnabled)}
            style={{
  ...
  const handleSaveSequence = (id, newName, newIsEnabled) => {
    const updatedSequences = sequences.map((sequence) =>
      sequence.id === id
        ? { ...sequence, name: newName, isEnabled: newIsEnabled }
        : sequence
    );
    ...
    try {
      ...
      if (updatedSequences.length > 0) {
        if (sequenceIndex === sequences.length - 1) {
          ...
          newActiveTabId = updatedSequences[sequenceIndex]
            ? updatedSequences[sequenceIndex].id
            : updatedSequences[0].id;
        }
      } else {
  ...
  const handleMenuClick = (event, sequenceId, stepId) => {
    ...
    const isCurrentMenuOpen =
      openMenu?.sequenceId === sequenceId && openMenu?.stepId === stepId;
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (
        openMenu !== null &&
        !event.target.closest(".email-step-header-right")
      ) {
        setOpenMenu(null);
      }
    };
  ...
  function DayCard({ day, intervals, setIntervals }) {
    const handleAddInterval = () => {
      if (intervals.length < 3) {
        const newId =
          intervals.length > 0 ? intervals[intervals.length - 1].id + 1 : 1;
    ...
    const handleDeleteInterval = (id) => {
      const updatedIntervals = intervals
        ...
        .map((interval, index) => ({ ...interval, id: index + 1 }));
          ...
          <div className="daycard-rows" key={interval.id}>
            <span style={{ display: "flex", alignItems: "center" }}>
              ...
              <CloseIcon
                ...
                sx={{ color: "red", marginLeft: 2, cursor: "pointer" }}
              />
            </span>
          </div>
        ))}
    ...
    const fetchSequencesDetails = async () => {
      try {
        ...
        if (sequences.length > 0) {
          setSequences(
            sequences.map((sequence) => ({ ...sequence, isNew: false }))
          );
  ...
  const verifySenderEmails = async () => {

    if (senderSelected.length > 0) {
      try {
        const response = await isValidSenderEmails({
        ...
        const unverifiedEmails = response.data
          .filter(item => !item.verified)
          .map(item => item.email);

        if (unverifiedEmails.length > 0) {
          alert(`The following emails are unverified:\n\n${unverifiedEmails.join('\n')}`);
  ...
  const handleTagChange = (e) => {
    ...
    const selectedTagObject =
      tags.find((tag) => tag.tags === selectedValue) || {};
  ...
  const handleStepClick = (stepId) => {
    ...
    if (isNewStepId(stepId)) {
      console.log(stepId);
      const containingSequence = sequences.find((seq) =>
        seq.steps.some((s) => s.id === stepId)
      );
      ...
      console.log(stepId);
      const containingSequence = sequences.find((seq) =>
        seq.steps.some((s) => s.step_id === stepId)
      );
      if (!containingSequence) return;

      const chosenStep = containingSequence.steps.find(
        (s) => s.step_id === stepId
      );
      if (!chosenStep) return;
        ...
        <div className="mailing-header">
          ...
          <div className="mailing-controls">
            ...
            <button
              ...
              disabled={isCampaignRunning || senderSelected.length==0}
          ...
          <TabContext value={value}>
            ...
            <TabPanel value="1">
              <div className="contact-info">
                    ...
                    >
                      ...
                      {tags.length > 0 ? (
                        tags.map((tagItem, index) => (
                          <MenuItem key={index} value={tagItem.tags}>
                            {tagItem.tags}
                          </MenuItem>
                        ))
                      ) : (
                ...
                <div
                  className="contacts-previews"
                  style={{ marginTop: "100px" }}
                >
                  <h2>Contacts preview</h2>
                  <p>
                    Segments are processed asynchronously and it may take some
                    time for contact to pop up in preview.
                  </p>
                  <TabContext value={value1}>
                    ...
                    <TabPanel value="1">
                      <TabContext value={value2}>
                        ...
                        <TabPanel value="2">Metrics</TabPanel>
                      </TabContext>
                    </TabPanel>
                  </TabContext>
                </div>
            ...
            <TabPanel value="2">
              <div className="mails-info">
                ...
                <p
                  style={{
                    color: selectedEmails2.length > 0 ? "green" : "red",
                  }}
                >
                  Selected {selectedEmails2.length} accounts for sending
                </p>
                <div style={{ height: 400, width: "50%" }}>
                  <DataGrid
                    ...
                    onRowSelectionModelChange={(newSelectionModel) => {
                      ...
                      const selectedEmails = newSelectionModel.map(
                        (index) => rows2[index]?.senderEmails || ""
                      );
            ...
            <TabPanel value="3">
              <div className="schedule-contanier">
                ...
                <div className="schedule-cards">
                  <Card
                    className={`card-container ${selectedCard === 0 ? "selected" : ""
                      }`}
                  ...
                  >
                    <CardContent>
                      <Typography
                        sx={{ fontWeight: 550, marginLeft: 15 }}
                        variant="h6"
                        component="div"
                      >
                        Working hours
                      ...
                      <Typography
                        sx={{ fontSize: 18, marginTop: 1, display: "flex" }}
                        color="text.secondary"
                        gutterBottom
                      >
                        <CheckCircleOutlineOutlinedIcon /> &nbsp; Monday to
                        friday
                      </Typography>
                      <Typography
                        sx={{ fontSize: 18, marginTop: 1, display: "flex" }}
                        ...
                        gutterBottom
                      >
                        <CheckCircleOutlineOutlinedIcon /> &nbsp; 8am to 1pm. 15
                        emails/mail account
                      </Typography>
                      <Typography
                        sx={{ fontSize: 18, marginTop: 1, display: "flex" }}
                        ...
                        gutterBottom
                      >
                        <CheckCircleOutlineOutlinedIcon /> &nbsp; 2pm to 7pm. 20
                        emails/mail account
                      </Typography>
                    </CardContent>
                  </Card>
                  ...
                  <Card
                    className={`card-container ${selectedCard === 1 ? "selected" : ""
                      }`}
                  ...
                  >
                    <CardContent>
                      ...
                      <Typography
                        ...
                        gutterBottom
                      >
                        <CheckCircleOutlineOutlinedIcon /> &nbsp; You select
                        days, time intervals and sending speed.
                      </Typography>
                    </CardContent>
                  </Card>
                </div>
            ...
            <TabPanel value="4">
              <TabContext value={activeSequenceTab}>
                ...
                >
                  ...
                  <Tooltip
                    title={
                      sequences.length >= 5
                        ? "Single campaign can have upto 5 sequences"
                        : "Create one more sequence"
                    }
                  >
                    <span>
                      <Button
              ...
              <EditSequenceDialog
                ...
                onToggleEnabled={() =>
                  toggleSequenceEnabled(currentSequence.id)
                }
              ...
              <div className="sequence-container">
                <div className="sequence-steps">
                  {activeSequence &&
                    activeSequence.steps.map((step, index) => (
                      <React.Fragment key={step.id || step.step_id}>
                        <div
                          className="email-step-box"
                          onClick={() =>
                            handleStepClick(step.id || step.step_id)
                          }
                        ...
                        >
                          <div className="email-step-header">
                            ...
                            <div className="email-step-header-right">
                              ...
                              {openMenu?.sequenceId === activeSequence.id &&
                                openMenu?.stepId ===
                                (step.step_id || step.id) && (
                                  <div
                                  ...
                                  >
                                    <div
                                      className="menu-item"
                                      onClick={() =>
                                        startRename(activeSequence.id, step.id)
                                      }
                                    >
                                    ...
                                    <div
                                      className="menu-item"
                                      onClick={() => {
                                        /* handleEditStep(sequence.id, step.id) */
                                      }}
                                    >
                                      <ModeEditIcon /> &nbsp; Edit this step
                                    </div>
                                    <div
                                      className="menu-item"
                                      onClick={() =>
                                  ...
                                  <div className="email-inputs-container">
                                    <>
                                      ...
                                      {index !== 0 && (
                                        ...
                                        >
                                          Subject not needed for follow up, it's
                                          always Re: [previous subject]
                                        </div>
                                      )}
                                      <CharacterCount
                                        text={step.subject}
                                    ...
                                    <>
                                      ...
                                      <CharacterCount
                                        text={cleanAndCountText(step.body)}
                                        maxLength={500}
                                      />
                                    </>
                                  </div>
                        ...
                        {index < activeSequence.steps.length - 1 && (
                          ...
                          >
                            <IconButton
                              onClick={() =>
                                updateDelay(step.step_id || step.id, false)
                              }
                              size="small"
                            >
                              <RemoveIcon />
                            ...
                            <IconButton
                              onClick={() =>
                                updateDelay(step.step_id || step.id, true)
                              }
                              size="small"
                            >
                              <AddIcon />
                  ...
                  <RenameDialog
                    ...
                    initialName={
                      renamingStepIndex !== null
                        ? emailSteps[renamingStepIndex].name ||
                        `Step ${renamingStepIndex + 1}`
                        : ""
                    }
                    onSave={(newName) => {
                      const updatedSteps = emailSteps.map((step, idx) =>
                        idx === renamingStepIndex
                          ? { ...step, name: newName }
                          : step
                      );
                      setEmailSteps(updatedSteps);
                ...
                <div className="sequence-controls">
                  <button
                    onClick={() => addEmailStepToSequence(activeSequence.id)}
